...
function toggleBookForm() {
  const isHidden = bookForm.classList.toggle("collapsed"); // .toggle() both add/removes 'collapsed' from classList AND returns a boolean on whether the string will be in the list after execution
  if (isHidden) {
...
function handleSubmit(event) {
  ...
  const newBook = {
    title: bookForm.title.value, // since the event.target IS the form, one could substitue bookForm here, but extracting the value from event.target is safer due to closure
